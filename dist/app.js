...
import {atTime,tideAt,displayTideAt,windWindows,directionName,castingNote,distanceMiles} from './domain.js';
...
function renderMap(){
 ...
 $('#map').querySelectorAll('[data-map-spot]').forEach(el=>{el.onclick=e=>{if(adding||dragged)return;e.stopPropagation();selectSpot(el.dataset.mapSpot)};el.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();selectSpot(el.dataset.mapSpot)}}});
 declutterMapLabels();
