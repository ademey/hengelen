...
import {atTime,tideAt,displayTideAt,windWindows,directionName,castingNote,distanceMiles,forecastTimeFor,observationSummary} from './domain.js';
...
function layoutLocation(){
 ...
 $('#detail-wind-limit').value=limit;$('#detail-limit-label').textContent=limit;
 inspector.querySelectorAll(':scope > .spot-title,:scope > h2,:scope > .location-sub').forEach(el=>el.remove());
 const notes=[...inspector.querySelectorAll(':scope > details')].find(el=>el.querySelector('summary')?.textContent==='Shore notes & casting');if(notes)notes.open=true;
}
...
function applyRoute(){
 ...
 const next=route==='journal'?'journal':id&&spots.some(s=>s.id===id)?'location':'atlas';
 ...
 $('#atlas-view').hidden=next!=='atlas';$('#location-view').hidden=next!=='location';$('#journal-view').hidden=next!=='journal';
 $('#atlas-tab').classList.toggle('selected',next!=='journal');$('#journal-tab').classList.toggle('selected',next==='journal');
 if(next==='journal')renderJournal();else{render();if(next==='location')loadDetails(selected)}
 if(changed){window.scrollTo(0,0);if(next==='location')$('#detail-title').focus({preventScroll:true})}
 document.title=next==='location'?getSpot().name+' — Hengelen':next==='journal'?'Journal — Hengelen':'Hengelen — Field atlas';
}
function switchView(journal){navigate(journal?'journal':'atlas')}
$('#atlas-tab').onclick=()=>switchView(false);$('#journal-tab').onclick=()=>switchView(true);
$('#back-atlas').onclick=()=>navigate('atlas');$('#detail-location').onchange=e=>selectSpot(e.target.value);
window.addEventListener('hashchange',applyRoute);
$('#detail-wind-limit').oninput=e=>{const input=$('#wind-limit');input.value=e.target.value;input.dispatchEvent(new Event('input'))};
$('#detail-wind-limit').onchange=()=>$('#wind-limit').dispatchEvent(new Event('change'));
