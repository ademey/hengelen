...
import {atTime,tideAt,displayTideAt,windWindows,directionName,distanceMiles,forecastTimeFor,observationSummary} from './domain.js';
const $=s=>document.querySelector(s);
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const read=(key,fallback)=>{try{return JSON.parse(localStorage.getItem(key))??fallback}catch{return fallback}};
let local={revision:0,spots:[],entries:[],prefs:{windLimit:10}},custom=[],entries=[],spots=[...presets],selected=null,hour=0,limit=10,zoom=1,panX=0,panY=0,dragged=false,mapDetail=null,adding=false,pending=null,coasts=[],stateReady=false,savePending=false,editingPin=null,editingTrip=null;
...
const atmoWindUrl=(spot,time)=>`https://sf.atmo.ai/wind@${spot.lat.toFixed(5)},${spot.lon.toFixed(5)},10.77,20,0,${Math.round(time)},1.50`;
...
function renderBanner(){const overviewId=selected??spots.find(s=>weatherById[s.id])?.id,m=weatherMeta[overviewId];let text='Loading weather and tide sources…';if(weatherById[overviewId])text=`${m?.status==='stale'?'Cached forecast · refresh unavailable':'Forecasts connected'} · weather retrieved ${m?.fetchedAt?clock(m.fetchedAt/1000):'—'} PT`;else if(overallError||(selected&&detailErrors[selected]))text='Weather unavailable · no sample values substituted';$('#data-status').textContent=text;$('#refresh').disabled=selected?detailLoading.has(selected):false;$('#refresh').textContent=selected&&detailLoading.has(selected)?'Loading…':'↻ Refresh';}
...
function renderMapPreview(){
 const host=$('#map-preview');if(!host)return;
 if(currentPage!=='atlas'||!selected){host.classList.remove('visible');host.setAttribute('aria-hidden','true');return}
...
function observationTags(o={}){return [[o.comfort&&`Casting ${o.comfort.replaceAll('-',' ')}`],[Number.isFinite(o.actualWind)&&`Wind ${fmt(o.actualWind,0)} kn`],[o.clarity&&`${o.clarity} water`],[o.bait&&`Bait ${o.bait}`],[Number.isFinite(o.encounters)&&`${o.encounters} fish encounter${o.encounters===1?'':'s'}`]].flat().filter(Boolean).map(value=>`<span>${esc(value)}</span>`).join('')}
function journalHistory(spot){const summary=observationSummary(entries,spot.name);return `<details><summary>Past trips · ${summary.trips} logged / ${summary.observed} with observations</summary><p>These records are shown for comparison only. They do not change session rankings.</p>${summary.averageWind===null?'':`<p>Average recorded wind: ${fmt(summary.averageWind,1)} kn across trips that include actual wind.</p>`}<div class="history-list">${summary.recent.map(entry=>`<article><small>${esc(entry.date)}${entry.time?' · '+esc(entry.time):''}</small><div class="observation-summary">${observationTags(entry.observations)}</div><p>${esc(entry.catch||'No encounter summary')}</p></article>`).join('')}</div></details>`}
...
$('#inspector').insertAdjacentHTML('beforeend',journalHistory(s));$('#log-selected').onclick=()=>openTrip();$('#inspector').querySelectorAll('[data-window]').forEach(b=>b.onclick=()=>jumpTo(Number(b.dataset.window)));if($('#edit-pin'))$('#edit-pin').onclick=()=>openPin(getSpot());}
...
function dismissAtlasPreview(){if(currentPage!=='atlas'||!selected)return;selected=null;focusedSource=null;render()}
$('#hour').oninput=e=>setHour(e.target.value);$('#now').onclick=()=>jumpTo(Date.now()/1000);$('#refresh').onclick=async()=>{await Promise.all([loadOverview(),...(selected?[loadDetails(selected,true)]:[])])};
...
$('#map').onclick=e=>{if(dragged)return;if(!adding){dismissAtlasPreview();return}const p=mapPoint(e);pending=unproject((p.x-400+400*zoom-panX)/zoom,(p.y-340+340*zoom-panY)/zoom);if(pending[1]<37.4||pending[1]>38.04||pending[0]<-122.85||pending[0]>-122.08){notice('Choose a point inside the atlas region.');return}openPin()};
...
document.addEventListener('pointerdown',e=>{if(currentPage==='atlas'&&selected&&!e.target.closest('#map-preview,[data-map-spot],[data-spot]'))dismissAtlasPreview()});
...
function openTrip(entry=null){if(!stateReady){notice('Local data is unavailable. Reload before saving a trip.');return}editingTrip=entry?.id??null;tripDraftVersion++;tripSnapshotPrefix=null;$('#trip-form').reset();const names=[...new Set([...spots.map(s=>s.name),...(entry?[entry.spot]:[])])];$('#trip-spot').innerHTML=names.map(name=>`<option ${name===(entry?.spot??getSpot().name)?'selected':''}>${esc(name)}</option>`).join('');$('#trip-date').value=entry?.date??pacificDate(timestamp());$('#trip-time').value=entry?entry.time||'':inputClock(timestamp());$('#trip-catch').value=entry?.catch??'';const observations=entry?.observations??{};$('#trip-comfort').value=observations.comfort??'';$('#trip-wind').value=observations.actualWind??'';$('#trip-clarity').value=observations.clarity??'';$('#trip-bait').value=observations.bait??'';$('#trip-encounters').value=observations.encounters??'';$('#trip-notes').value=entry?.notes??tripForecastNote();if(!entry)tripSnapshotPrefix=forecastPrefix($('#trip-notes').value);$('#trip-dialog').showModal()}
...
setInterval(()=>{if(document.hidden)return;loadOverview();if(selected)loadDetails(selected,true)},10*60000);
