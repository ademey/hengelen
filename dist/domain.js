...
export function validateState(input){if(!input||!Array.isArray(input.spots)||!Array.isArray(input.entries))throw Error('Invalid local data');if(input.spots.length>500||input.entries.length>5000)throw Error('Local record limit exceeded');const str=(s,n,required=false)=>{if(typeof s!=='string'||s.length>n||(required&&!s.trim()))throw Error('Invalid text field');return s.trim()};const id=s=>{s=str(s,100,true);if(!/^[\w-]+$/.test(s))throw Error('Invalid record ID');return s};const unique=rows=>{if(new Set(rows.map(r=>r.id)).size!==rows.length)throw Error('Duplicate record ID')};const spots=input.spots.map(s=>{if(!Number.isFinite(s.lat)||!Number.isFinite(s.lon)||s.lat<37.4||s.lat>38.04||s.lon< -122.85||s.lon> -122.08)throw Error('Pin is outside the atlas region');return {id:id(s.id),name:str(s.name,60,true),notes:str(s.notes??'',1000),lat:s.lat,lon:s.lon,referenceId:str(s.referenceId??'crissy',40,true),region:'YOUR SPOTS',exposure:s.exposure==='Open coast'?'Open coast':'Bay shoreline'}});const entries=input.entries.map(e=>({id:id(e.id),spot:str(e.spot,100,true),date:str(e.date,10,true),catch:str(e.catch??'',160),notes:str(e.notes??'',4000)}));if(entries.some(e=>!/^\d{4}-\d{2}-\d{2}$/.test(e.date)||(!Number.isFinite(Date.parse(e.date))||new Date(e.date).toISOString().slice(0,10)!==e.date)))throw Error('Invalid trip date');unique(spots);unique(entries);const limit=number(input.prefs?.windLimit??10);if(limit===null||limit<5||limit>20)throw Error('Wind preference must be 5–20 knots');return {spots,entries,prefs:{windLimit:limit}}}
