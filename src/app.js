...
let forecastTimes = [],
  ...
  detailErrors = {},
  overallError = null;
const pacificDate = (t = Date.now() / 1000) =>
...
const atmoWindUrl = (spot, time) =>
  `https://sf.atmo.ai/wind@${spot.lat.toFixed(5)},${spot.lon.toFixed(5)},10.77,20,0,${Math.round(time)},1.50`;
function dayTitle(t) {
  const today = pacificDate();
  return pacificDate(t) === today ? 'Today' : `${weekday(t)} ${shortDate(t)}`;
}
function conditions(s, h = hour) {
  return (
    atTime(weatherById[s.id]?.hours, forecastTimes[h] ?? timestamp()) ?? {
      wind: null,
      gust: null,
      direction: null,
      rain: null,
...
function nearestHour(time) {
  ...
  forecastTimes.forEach((t, i) => {
    if (Math.abs(t - time) < Math.abs(forecastTimes[best] - time)) best = i;
  });
  return best;
}
...
function jumpTo(time) {
  hour = nearestHour(time);
  render();
}
function renderBanner() {
  const overviewId = selected ?? spots.find((s) => weatherById[s.id])?.id,
    m = weatherMeta[overviewId];
  let text = 'Loading weather and tide sources…';
  if (weatherById[overviewId])
    text = `${m?.status === 'stale' ? 'Cached forecast · refresh unavailable' : 'Forecasts connected'} · weather retrieved ${m?.fetchedAt ? clock(m.fetchedAt / 1000) : '—'} PT`;
  else if (overallError || (selected && detailErrors[selected]))
    text = 'Weather unavailable · no sample values substituted';
  $('#data-status').textContent = text;
  $('#refresh').disabled = selected ? detailLoading.has(selected) : false;
  $('#refresh').textContent = selected && detailLoading.has(selected) ? 'Loading…' : '↻ Refresh';
}
function renderList() {
  let region = '';
  $('#spot-list').innerHTML = spots
    .map((s, i) => {
      const header =
        s.region !== region
          ? `<div class="region-title"><span>${esc(s.region)}</span><span>↗</span></div>`
          : '';
      region = s.region;
      const c = conditions(s);
      return (
        header +
        `<button class="spot-row ${s.id === selected ? 'active' : ''}" data-spot="${s.id}" aria-pressed="${s.id === selected}" title="${esc(s.name)} · forecast wind"><span class="code">${String(i + 1).padStart(2, '0')}</span><span>${esc(s.name)}</span><span class="reading">${fmt(c.wind, 0)} kn</span></button>`
      );
    })
    .join('');
  $('#spot-list')
...
const project = (lon, lat) => {
  ...
  let y = ((38.04 - lat) / 0.64) * 640;
  return [45 + x * 0.91 - y * 0.1, 18 + y * 0.93 + x * 0.065];
};
const unproject = (x, y) => {
  let a = x - 45,
    b = y - 18;
  let yy = (b - (a * 0.065) / 0.91) / (0.93 + (0.1 * 0.065) / 0.91),
    xx = (a + 0.1 * yy) / 0.91;
  return [-122.85 + (xx / 780) * 0.77, 38.04 - (yy / 640) * 0.64];
};
const path = (poly) =>
  poly
    .map(
...
function selectedSources() {
  ...
  if (reference?.tideStation) {
    ...
    sources.push({
      ...
      code: 'T',
      kind: 'station',
      label: 'Tide prediction reference',
      description: t.continuous ? 'NOAA tide curve + high/low events' : 'NOAA high/low events only',
    });
  }
  if (reference?.currentStation) {
    const c = reference.currentStation;
    sources.push({
      ...c,
      key: 'current-' + c.id,
      code: 'C',
      kind: 'station',
      label: 'Current prediction reference',
      description: 'NOAA flood / ebb / slack events; not a live current sensor',
    });
  }
  const same = sources.find((p) => p.key === 'noaa-9414290');
  if (same) {
    same.code = 'T/O';
    same.label = 'Tides + observed wind';
    same.description += ' · regional wind observations';
  } else
    sources.push({
      key: 'noaa-9414290',
      id: '9414290',
      code: 'O',
      name: 'San Francisco / Torpedo Wharf',
      lat: 37.8063,
      lon: -122.4659,
      kind: 'station',
      label: 'Observed wind reference',
      description: 'NOAA regional observations, not wind measured at your fishing spot',
    });
  ...
  if (weather)
    sources.push({
      ...
      key: 'weather-grid',
      code: 'W',
      name: 'Wind forecast grid',
      kind: 'grid',
      label: 'Modeled wind, gusts & rain',
      description: 'Open-Meteo model grid center; not a physical NOAA station',
    });
  ...
  if (marine)
    sources.push({
      ...
      key: 'marine-grid',
      code: 'S',
      name: 'Offshore swell forecast grid',
      kind: 'grid',
      label: 'Modeled offshore waves',
      description: 'Open-Meteo marine model grid center; not a buoy or beach measurement',
    });
  return sources.map((p) => ({ ...p, distance: distanceMiles(spot, p) }));
}
function focusSource(key) {
  focusedSource = key;
  const source = selectedSources().find((s) => s.key === key);
  if (!source) return;
  const spot = getSpot(),
    a = project(spot.lon, spot.lat),
    b = project(source.lon, source.lat);
  zoom = Math.min(3, Math.max(1, 360 / Math.max(Math.abs(a[0] - b[0]), Math.abs(a[1] - b[1]), 1)));
  panX = (400 - (a[0] + b[0]) / 2) * zoom;
  panY = (340 - (a[1] + b[1]) / 2) * zoom;
  renderMap();
}
function sourceMapLayer() {
  const host = $('#source-strip');
  host.hidden = !showSources;
  $('#source-toggle').setAttribute('aria-pressed', String(showSources));
  if (!showSources) return '';
  const spot = getSpot(),
    sources = selectedSources(),
    [sx, sy] = project(spot.lon, spot.lat);
  ...
  host
    .querySelectorAll('[data-source-key]')
    .forEach((b) => (b.onclick = () => focusSource(b.dataset.sourceKey)));
  const lines = sources
    .map((p) => {
      const [x, y] = project(p.lon, p.lat);
      return `<path d="M${sx} ${sy}L${x} ${y}" class="source-connection ${p.kind} ${focusedSource === p.key ? 'focused' : ''}"/>`;
    })
    .join('');
  const pins = sources
    .map((p) => {
      const [x, y] = project(p.lon, p.lat),
        active = focusedSource === p.key;
      return `<g role="button" tabindex="0" class="source-marker ${p.kind} ${active ? 'focused' : ''}" data-source-marker="${p.key}" transform="translate(${x} ${y}) scale(${(currentPage === 'location' ? 2 : mapAnnotationSize) / zoom})" aria-label="${esc(p.name)}, ${esc(p.label)}"><title>${esc(p.name)} — ${esc(p.label)}</title><circle r="10" fill="transparent"/>${p.kind === 'grid' ? '<path d="M0 -6l6 6 -6 6 -6 -6Z" class="source-symbol"/>' : '<rect x="-5" y="-5" width="10" height="10" class="source-symbol"/>'}</g>`;
    })
    .join('');
  return `<g class="source-layer">${lines}${pins}</g>`;
}
function mapTransform() {
  return `translate(${400 - 400 * zoom + panX} ${340 - 340 * zoom + panY}) scale(${zoom})`;
}
function renderMap() {
  const sourceLayer = currentPage === 'location' ? sourceMapLayer() : '';
  const land = coasts.map((p) => `<path d="${path(p)}"/>`).join('');
  const textAt = (lon, lat, text, cls = 'map-label', dx = 0, dy = 0) => {
    const [x, y] = project(lon, lat);
    return `<g transform="translate(${x} ${y}) scale(${(currentPage === 'location' ? 2 : mapAnnotationSize) / zoom})"><text x="${dx}" y="${dy}" text-anchor="middle" class="${cls}">${esc(text)}</text></g>`;
  };
  const labels = [
    [-122.68, 37.995, 'MARIN'],
    [-122.43, 37.72, 'SAN FRANCISCO'],
    [-122.18, 37.945, 'EAST BAY'],
    [-122.735, 37.67, 'PACIFIC'],
    [-122.735, 37.645, 'OCEAN'],
    [-122.335, 37.705, 'SAN FRANCISCO'],
    [-122.335, 37.68, 'BAY'],
  ]
    .map((p) => textAt(...p))
    .join('');
  const terrain = (mapDetail?.contours ?? [])
    .filter((c) => zoom >= 1.5 || c.meters % 100 === 0)
    .map(
      (c) =>
        `<path d="${c.d}" class="terrain-contour ${c.meters % 200 === 0 ? 'index-contour' : ''}"/>`,
    )
    .join('');
  const roads = (mapDetail?.roads ?? [])
    .map((r) => `<path d="${r.d}" class="${r.bridge ? 'bridge-line' : 'road-line'}"/>`)
    .join('');
  const bridgeLabels = [
    [-122.481, 37.823, 'GOLDEN GATE', -53, -6],
    [-122.373, 37.803, 'BAY BRIDGE', 38, 16],
    [-122.441, 37.936, 'RICHMOND–SAN RAFAEL', 0, -7],
  ]
    .map(([a, b, t, x, y]) => textAt(a, b, t, 'landmark-label bridge-label', x, y))
    .join('');
  const islands = [
    [-122.432, 37.827, 'ALCATRAZ', 32, -10],
    [-122.432, 37.862, 'ANGEL ISLAND', 38, -10],
    [-122.371, 37.823, 'TREASURE IS.', 38, -9],
  ]
    .map(([a, b, t, dx, dy]) => {
      let [x, y] = project(a, b);
      return `<g class="island-label" transform="translate(${x} ${y}) scale(${(currentPage === 'location' ? 2 : mapAnnotationSize) / zoom})"><path d="M0 0l${dx * 0.5} ${dy}" fill="none" stroke="#78745c" stroke-width=".6"/><circle r="1.7" fill="#555640"/><text x="${dx}" y="${dy - 3}" text-anchor="middle" class="landmark-label">${t}</text></g>`;
    })
    .join('');
  const peaks = [
    [-122.596, 37.923, 'MT. TAMALPAIS'],
    [-122.451, 37.69, 'SAN BRUNO MTN.'],
    [-122.477, 37.578, 'MONTARA MTN.'],
  ]
    .map(([a, b, name]) => {
      let [x, y] = project(a, b);
      return `<g transform="translate(${x} ${y}) scale(${(currentPage === 'location' ? 2 : mapAnnotationSize) / zoom})"><path d="M-3 2l3 -5 3 5Z" fill="#777257"/><text y="-9" text-anchor="middle" class="landmark-label peak-label">${name}</text></g>`;
    })
    .join('');
  const localLabels = [
    [-122.505, 37.85, 'SAUSALITO'],
    [-122.456, 37.887, 'TIBURON'],
    [-122.253, 37.813, 'OAKLAND'],
    [-122.503, 37.771, 'GOLDEN GATE PARK'],
    [-122.465, 37.79, 'PRESIDIO'],
    [-122.49, 37.51, 'PILLAR POINT'],
  ]
    .map((p) => textAt(...p, 'landmark-label local-label'))
    .join('');
  const winds = presets
    .filter((s) => ['stinson', 'crissy', 'hmb'].includes(s.id))
    .map((s) => {
      const c = conditions(s);
      if (c.direction === null || c.wind === null) return '';
      let [x, y] = project(s.lon, s.lat);
      return `<g class="map-wind" transform="translate(${x} ${y}) scale(${(currentPage === 'location' ? 2 : mapAnnotationSize) / zoom})"><g transform="translate(-66 24)"><g transform="rotate(${c.direction + 180})"><path d="M0 13V-13m-4 5 4 -5 4 5" fill="none" stroke="#565a40" stroke-width="1.2"/></g><text x="13" y="3" font-size="8" fill="#686650">${fmt(c.wind, 0)} kn</text></g></g>`;
    })
    .join('');
  const markers = spots
    .map((s, i) => {
      ...
      let right = ['crissy', 'richmond', 'berkeley', 'alameda', 'candlestick', 'coyote'].includes(
        s.id,
      );
      let labelVisible = active || (currentPage !== 'location' && zoom >= 1.7);
      return `<g class="marker ${currentPage === 'atlas' && selected && !active ? 'muted-marker' : ''}" role="button" tabindex="0" aria-label="${currentPage === 'atlas' ? 'Preview' : 'Select'} ${esc(s.name)}" aria-pressed="${active}" data-map-spot="${s.id}" transform="translate(${x} ${y}) scale(${(currentPage === 'location' ? 2 : mapAnnotationSize) / zoom})">${active ? '<circle class="selection-ring outer" r="23"/><circle class="selection-ring" r="17"/>' : ''}<circle class="marker-dot" r="${active ? 7 : 5}" fill="${active ? '#25271b' : '#eee8cf'}" stroke="#25271b" stroke-width="1.4"/><path d="M${right ? 9 : -9} 0h${right ? 17 : -17}" stroke="#555640" stroke-width=".7"/><rect x="${right ? 26 : -51}" y="-10" width="25" height="20" fill="${active ? '#25271b' : '#e6dfbe'}" stroke="#87836c" stroke-width=".7"/><text x="${right ? 38 : -38}" y="4" text-anchor="middle" fill="${active ? '#e6dfbe' : '#25271b'}">${String(i + 1).padStart(2, '0')}</text>${labelVisible ? `<text class="spot-map-name" x="${right ? 58 : -58}" y="4" text-anchor="${right ? 'start' : 'end'}" fill="#25271b">${esc(s.name)}</text>` : ''}</g>`;
    })
    .join('');
  $('#map-content').innerHTML =
    `<defs><clipPath id="land-clip">${land}</clipPath><mask id="water-mask"><rect x="-1000" y="-1000" width="3000" height="3000" fill="white"/><g fill="black">${land}</g></mask></defs><g id="geography" transform="${mapTransform()}"><g fill="none" stroke="#77795e" opacity=".14" mask="url(#water-mask)" stroke-width="14">${land}</g><g fill="none" stroke="#77795e" opacity=".2" mask="url(#water-mask)" stroke-width="6">${land}</g><g transform="translate(0 7)" fill="url(#hatch)" stroke="#868168" stroke-width=".6">${land}</g><g fill="#e6dfbe" stroke="#8b866c" stroke-width=".8">${land}</g><g clip-path="url(#land-clip)">${terrain}</g><g class="road-layer">${roads}</g></g><g id="map-annotations" transform="${mapTransform()}">${labels}<g class="geographic-annotations ${zoom >= 1.5 ? 'close-detail' : ''}">${bridgeLabels}${islands}${peaks}${zoom >= 1.5 ? localLabels : ''}</g>${winds}${markers}${sourceLayer}</g>`;
  $('#map')
  ...
  $('#map')
    .querySelectorAll('[data-map-spot]')
    .forEach((el) => {
      const choose = () =>
...
function renderMapPreview() {
  ...
  const s = getSpot(),
    c = conditions(s),
    detail = detailsById[s.id],
    tide = displayTideAt(detail, timestamp()),
    weatherState = weatherMeta[s.id]?.status;
  const wind = Number.isFinite(c.wind)
    ? `${fmt(c.wind, 0)} kn · gusts ${fmt(c.gust, 0)} kn`
    : 'Forecast unavailable';
  const tideText = tide
    ? `${tide.estimated ? 'Estimated ' : 'Predicted '}${fmt(tide.value)} ft · ${tide.rising ? 'rising' : 'falling'}`
    : detail
      ? 'Tide unavailable'
...
function declutterMapLabels() {
  ...
  const intersects = (a, b) =>
    a.left < b.right + 4 && a.right > b.left - 4 && a.top < b.bottom + 4 && a.bottom > b.top - 4;
  const visible = (r) =>
    r.right > bounds.left &&
    r.left < bounds.right &&
    r.bottom > bounds.top &&
    r.top < bounds.bottom;
  map.querySelectorAll('.marker-dot,.marker rect,.source-symbol').forEach((el) => {
    const r = el.getBoundingClientRect();
    if (visible(r)) occupied.push(r);
  });
  const selectedName = map.querySelector(`[data-map-spot="${selected}"] .spot-map-name`);
  const names = [selectedName, ...map.querySelectorAll('.spot-map-name')].filter(
    (el, i, all) => el && all.indexOf(el) === i,
  );
  const labels = [...names, ...map.querySelectorAll('.map-label,.landmark-label,.map-wind')];
  labels.forEach((el) => {
    el.style.visibility = '';
    const r = el.getBoundingClientRect();
    if (!visible(r) || occupied.some((other) => intersects(r, other)))
...
function lineChart(rows, key, options = {}) {
  if (!rows?.length || !rows.some((r) => Number.isFinite(r[key])))
    return '<p class="empty-chart">No readings for this day.</p>';
  const vals = rows
    .flatMap((r) => [r[key], options.secondary ? r[options.secondary] : null])
    .filter(Number.isFinite);
  let lo = options.zero ? 0 : Math.floor(Math.min(...vals)) - 1,
    hi = Math.ceil(Math.max(...vals)) + 1;
  if (options.threshold) hi = Math.max(hi, options.threshold + 2);
  const wide = Boolean(options.width),
    width = options.width ?? 292,
    height = options.height ?? 105;
  const left = wide ? 34 : 16,
    right = width - (wide ? 10 : 12),
    top = wide ? 18 : 17,
    bottom = height - (wide ? 30 : 23);
  const x = (t) =>
      left +
      ((t - rows[0].time) / Math.max(3600, rows.at(-1).time - rows[0].time)) * (right - left),
  ...
  const draw = (k) => {
    ...
    return rows
      .map((r) => {
        ...
        const segment = `${connected ? 'L' : 'M'}${x(r.time).toFixed(1)},${y(r[k]).toFixed(1)}`;
        connected = true;
        return segment;
      })
      .join(' ');
  ...
  const tick = timestamp(),
    ticks = wide ? [lo, (lo + hi) / 2, hi] : [lo, hi];
  const labels =
    wide && width >= 440
      ? [
          rows[0],
...
const dayRows = (rows) =>
  (rows ?? []).filter((r) => pacificDate(r.time) === pacificDate(timestamp()));
function sourceLine(label, resource, url) {
...
function renderInspector() {
  const s = getSpot(),
    ...
    c = conditions(s),
    t = timestamp(),
    tide = displayTideAt(data, t),
    marine = atTime(data?.marine.value?.hours, t),
    known = Number.isFinite(c.wind),
    fresh = weatherMeta[s.id]?.status !== 'stale',
    idx = spots.indexOf(s) + 1;
  const gustOkay = Number.isFinite(c.gust) && c.gust <= limit + 3;
  let title = !known
    ...
    : c.wind > limit
      ...
      : !gustOkay
        ? 'Watch the gusts'
        : 'Within your wind preference';
  let explanation = !known
    ? detailErrors[s.id] || 'The forecast will appear here when the weather source responds.'
    : c.wind > limit
      ? `Forecast wind exceeds your ${limit}-knot comfort setting. Compare earlier hours or other shorelines.`
      : !gustOkay
        ? `Average wind is within your setting, but ${c.gust === null ? 'gust data is missing' : 'gusts reach ' + fmt(c.gust, 0) + ' knots'}. Treat the average alone with care.`
        : `Forecast wind and gusts fit your starting preferences. ${c.day ? 'This hour is in daylight.' : 'This hour is after dark.'}`;
  if (!fresh) explanation += ' This is cached data; the latest refresh failed.';
  const nextCurrent = data?.currents.value?.find((e) => e.time >= t),
    tideEvents = (data?.highLow.value ?? []).filter((e) => e.time >= t).slice(0, 4),
    currentEvents = (data?.currents.value ?? []).filter((e) => e.time >= t).slice(0, 4),
    windows = planningWindows(s);
  const obs = data?.observedWind.value,
    obsOld = obs && (Date.now() / 1000 - obs.time > 3600 || data.observedWind.status === 'stale'),
    temp = data?.observedTemperature.value;
  const station = data?.spot.tideStation,
    curStation = data?.spot.currentStation;
...
function renderTime() {
  const list = days(),
    date = pacificDate(timestamp()),
    selectedDay = list.find((d) => d.key === date);
  $('#time-label').textContent = `${dayTitle(timestamp())} · ${clock(timestamp())}`;
  $('#days').innerHTML = list
    .map(
      (d) =>
        `<button data-time="${d.t}" class="${date === d.key ? 'selected' : ''}">${d.key === pacificDate() ? 'Today' : weekday(d.t)}<small>${shortDate(d.t)}</small></button>`,
    )
    .join('');
  $('#days')
    ...
    .forEach(
      (b) =>
        (b.onclick = () => {
          const oldDate = days().find((d) => d.key === pacificDate(timestamp()));
          jumpTo(Number(b.dataset.time) + (oldDate ? timestamp() - oldDate.t : 0));
        }),
    );
  const indices = forecastTimes.map((t, i) => ({ t, i })).filter((o) => pacificDate(o.t) === date);
  $('#hour').disabled = !indices.length;
  $('#hour').min = indices[0]?.i ?? 0;
  $('#hour').max = indices.at(-1)?.i ?? 23;
  $('#hour').value = hour;
  $('#timeline-note').textContent =
    weatherMeta[selected]?.status === 'stale'
      ? 'Cached forecast · refresh unavailable'
      : 'Shared time axis · Pacific · click the chart or move the slider';
...
async function loadDetails(id, force = false) {
  if (
    detailLoading.has(id) ||
    (!force && detailsById[id] && Date.now() - detailsById[id].servedAt < 5 * 60000)
  )
    return;
  detailLoading.add(id);
  if (selected === id) renderBanner();
  try {
    const d = await api('/api/spot?id=' + encodeURIComponent(id));
    detailsById[id] = d;
    if (d.weather.value) {
      weatherById[id] = d.weather.value;
      weatherMeta[id] = d.weather;
      setTimes(d.weather.value.hours.map((r) => r.time));
    }
    delete detailErrors[id];
  } catch (e) {
    detailErrors[id] = e.message;
  } finally {
...
function setTimes(times) {
  if (!forecastTimes.length) {
    forecastTimes = times;
    hour = nearestHour(Date.now() / 1000);
  } else if (forecastTimes[0] !== times[0]) {
    const previous = timestamp();
    forecastTimes = times;
    hour = nearestHour(previous);
  }
}
async function loadOverview() {
  try {
    const d = await api('/api/overview');
    if (!d.value) throw Error(d.error || 'Forecast unavailable');
    for (const [id, value] of Object.entries(d.value)) {
      weatherById[id] = value;
      weatherMeta[id] = { status: d.status, fetchedAt: d.fetchedAt };
    }
    setTimes(Object.values(d.value)[0].hours.map((r) => r.time));
    overallError = null;
  } catch (e) {
...
function selectSpot(id) {
  ...
  navigate('spot/' + encodeURIComponent(id));
}
...
function dismissAtlasPreview() {
  if (currentPage !== 'atlas' || !selected) return;
  selected = null;
  focusedSource = null;
  render();
}
$('#hour').oninput = (e) => setHour(e.target.value);
$('#now').onclick = () => jumpTo(Date.now() / 1000);
$('#refresh').onclick = async () => {
  await Promise.all([loadOverview(), ...(selected ? [loadDetails(selected, true)] : [])]);
};
$('#wind-limit').oninput = (e) => {
...
$('#zoom-in').onclick = () => changeZoom(zoom + 0.35);
$('#zoom-out').onclick = () => changeZoom(zoom - 0.35);
$('#reset-map').onclick = () => {
  if (currentPage === 'location') {
    const [x, y] = project(getSpot().lon, getSpot().lat);
    zoom = 2.6;
    panX = (400 - x) * zoom;
    panY = (340 - y) * zoom;
  } else {
...
$('#map').addEventListener('pointermove', (e) => {
  ...
  if (dragged) {
    ...
    $('#geography').setAttribute('transform', mapTransform());
    $('#map-annotations').setAttribute('transform', mapTransform());
...
function layoutLocation() {
  ...
  const notes = [...inspector.querySelectorAll(':scope > details')].find(
    (el) => el.querySelector('summary')?.textContent === 'Shore notes',
  );
...
function applyRoute() {
  ...
  if (next === 'location') {
    ...
    if (currentPage !== 'atlas' && atlasCamera) {
      ({ zoom, panX, panY } = atlasCamera);
      atlasCamera = null;
    }
    if (currentPage !== 'atlas') selected = null;
  }
  currentPage = next;
  ...
  if (changed) {
    window.scrollTo(0, 0);
    if (next === 'location') $('#detail-title').focus({ preventScroll: true });
  }
  document.title =
    next === 'location'
...
document.addEventListener('pointerdown', (e) => {
  if (
    currentPage === 'atlas' &&
    selected &&
    !e.target.closest('#map-preview,[data-map-spot],[data-spot]')
  )
    dismissAtlasPreview();
});
...
fetch('coast.json')
  ...
  .catch(() => notice('Coastline unavailable. Refresh to try again.'));
fetch('detail.json')
  .then((r) => {
    if (!r.ok) throw Error();
    return r.json();
  ...
  .catch(() => notice('Terrain detail unavailable; base coastline still shown.'));
setInterval(() => {
  if (document.hidden) return;
  loadOverview();
  if (selected) loadDetails(selected, true);
}, 10 * 60000);

function planningWindows(spot = getSpot(), from = Date.now() / 1000) {
  return distinctWindows(
    fishingWindows({
      weather: weatherById[spot.id],
      weatherStatus: weatherMeta[spot.id]?.status,
      detail: detailsById[spot.id],
      spot,
      limit,
      now: from,
    }).filter((w) => pacificDate(w.start) === pacificDate(timestamp())),
    3,
  );
}
function windowCards(windows) {
  if (!windows.length)
    return '<p class="empty-chart">No matching two-hour sessions on this day. Try another day or spot. Fresh wind/gust data and a full daylight interval are required.</p>';
  return windows
    .map(
      (w) =>
        `<article class="session-card"><button data-window="${w.start}"><span>${clock(w.start)}–${clock(w.end)}<small>${w.provisional ? 'PROVISIONAL · DATA GAPS' : 'WORTH CONSIDERING'}</small></span><span>↗</span></button><ul>${w.reasons.map((r) => `<li>${esc(r)}</li>`).join('')}</ul>${w.cautions.length ? `<p class="session-tradeoff">${w.cautions.map(esc).join(' · ')}</p>` : ''}${w.missing.length ? `<p class="session-missing">Missing context: ${w.missing.map(esc).join(' · ')}</p>` : ''}</article>`,
    )
    .join('');
}
function renderConditionsChart() {
  const host = $('#conditions-chart'),
    width = Math.max(1, host.clientWidth);
  if (width < 2) return;
  const detail = detailsById[selected],
    weather = weatherById[selected];
  host.innerHTML = conditionsTimeline({
    hours: dayRows(weather?.hours),
    tides: detail?.tides.value ?? [],
    highLow: detail?.highLow.value ?? [],
    currents: detail?.currents.value ?? [],
    sun: weather?.days ?? [],
    windows: planningWindows(),
    selected: timestamp(),
    limit,
    width,
    tideStatus: detail?.tides.status,
    highLowStatus: detail?.highLow.status,
    currentStatus: detail?.currents.status,
    currentStation: detail?.spot.currentStation?.name,
    weatherStatus: weatherMeta[selected]?.status,
  });
  host.onclick = (e) => {
...
function renderComparison() {
  const host = $('#spot-comparison');
  if (!host || !comparisonOpen) return;
  host.innerHTML = `<p class="comparison-heading">${dayTitle(timestamp())} · ${clock(timestamp())}–${clock(timestamp() + 7200)}${comparisonLoading ? ' · loading references…' : ''}</p><div class="comparison-grid">${spots
    .map((spot) => {
      const w = weatherById[spot.id],
        detail = detailsById[spot.id],
        c = atTime(w?.hours, timestamp());
      const candidate = fishingWindows({
        weather: w,
        detail,
        spot,
        limit,
        weatherStatus: weatherMeta[spot.id]?.status,
        now: timestamp(),
      }).find((r) => r.start === timestamp());
      let summary = candidate
        ? candidate.reasons.join(' · ')
        : !c
          ? 'Forecast unavailable'
          : weatherMeta[spot.id]?.status !== 'ok'
            ? 'Fresh forecast unavailable'
            : `No matching two-hour daylight session within your wind limits · wind ${fmt(c.wind, 0)} / gusts ${fmt(c.gust, 0)} kn`;
    ...
    .join('')}</div>`;
  host
    .querySelectorAll('[data-compare-spot]')
    .forEach((b) => (b.onclick = () => selectSpot(b.dataset.compareSpot)));
}
$('#compare-spots').onclick = async () => {
  comparisonOpen = !comparisonOpen;
  $('#spot-comparison').hidden = !comparisonOpen;
  $('#compare-spots').setAttribute('aria-expanded', String(comparisonOpen));
  if (!comparisonOpen) return;
  comparisonLoading = true;
  renderComparison();
  const pending = spots.map((s) => s.id);
  await Promise.all(
    Array.from({ length: 3 }, async () => {
      while (pending.length) {
        await loadDetails(pending.shift());
        renderComparison();
      }
    }),
  );
...
new ResizeObserver(() => {
  ...
  chartResizeFrame = requestAnimationFrame(renderConditionsChart);
}).observe($('#conditions-chart'));
