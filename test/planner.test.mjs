...
const weather={hours:Array.from({length:12},(_,i)=>({time:i*3600,day:true,wind:5,gust:7,direction:270})),days:[{sunrise:0,sunset:43200}]};
const detail={tides:{status:'ok',value:Array.from({length:121},(_,i)=>({time:i*360,value:i/20}))},highLow:{status:'ok',value:[]},currents:{status:'ok',value:[{time:0,type:'slack'},{time:43200,type:'slack'}]},alerts:{status:'ok',value:[]}};
const input={weather,detail,spot:{exposure:'Bay shoreline',bearing:0},limit:10,now:0};
test('sessions cover two complete daylight hours and respect missing/gapped forecasts',()=>{
 const windows=fishingWindows(input);assert.ok(windows.length);assert.ok(windows.every(w=>w.end<=43200&&w.end-w.start===7200));
 assert.equal(fishingWindows({...input,weatherStatus:'stale'}).length,0);
 assert.equal(fishingWindows({...input,weather:{...weather,hours:[weather.hours[0],weather.hours[2]]}}).length,0);
 assert.equal(fishingWindows({...input,weather:{...weather,hours:weather.hours.map(r=>({...r,gust:null}))}}).length,0);
 assert.equal(fishingWindows({...input,weather:{...weather,days:[{sunrise:0,sunset:3500}]}}).length,0);
});
test('missing references are provisional and stale tide is not used for ranking',()=>{
 const missing=fishingWindows({...input,detail:undefined});assert.ok(missing.every(w=>w.provisional&&w.missing.length));
 const stale=fishingWindows({...input,detail:{...detail,tides:{...detail.tides,status:'stale'}}});assert.ok(stale.every(w=>!w.reasons.some(r=>r.startsWith('Tide '))));
 const good=fishingWindows(input);assert.ok(good.every(w=>!w.provisional));assert.ok(good[0].lowLight);
});
test('suggestions do not overlap and ocean swell gaps remain explicit',()=>{
 const windows=distinctWindows(fishingWindows(input));assert.equal(windows.length,3);
