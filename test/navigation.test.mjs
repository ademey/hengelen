...
function setup() {
  ...
  const context = vm.createContext({
    ...
    renderJournal() {},
    loadDetails(id) {
      context.loaded.push(id);
    },
    project: () => [300, 250],
    getSpot: () => ({ lon: 0, lat: 0, name: 'Selected shoreline' }),
    history: {
      pushState(_a, _b, hash) {
...
test('atlas marker preview changes selection without changing the camera or route', () => {
  ...
  vm.runInContext("previewSpot('kirby')", c);
  assert.equal(c.selected, 'kirby');
  assert.equal(c.location.hash, '');
  assert.equal(c.zoom, 1.7);
  assert.equal(c.panX, 42);
  assert.equal(c.panY, -18);
  assert.equal(c.renders, 1);
  assert.deepEqual([...c.loaded], ['kirby']);
});
...
test('spot navigation opens details and returning restores the atlas camera', () => {
  const { context: c, $ } = setup();
  vm.runInContext("navigate('spot/kirby')", c);
  assert.equal(c.currentPage, 'location');
  assert.equal(c.selected, 'kirby');
  assert.equal($('#atlas-view').hidden, true);
  assert.equal($('#location-view').hidden, false);
  assert.equal($('#detail-map-slot').child, $('#map-panel'));
  vm.runInContext("navigate('atlas')", c);
  assert.equal(c.selected, null);
  assert.equal(c.zoom, 1.7);
  assert.equal(c.panX, 42);
  assert.equal(c.panY, -18);
...
test('invalid location links fall back to the atlas without crashing', () => {
  const { context: c } = setup();
  for (const hash of ['#spot/missing', '#spot/%invalid']) {
    c.location.hash = hash;
    vm.runInContext('applyRoute()', c);
    assert.equal(c.currentPage, 'atlas');
  }
});
test('Atmo wind links preserve the selected spot and forecast time and open safely', () => {
  const context = vm.createContext({});
  const start = app.indexOf('const atmoWindUrl');
  vm.runInContext(app.slice(start, app.indexOf(';\n', start) + 1), context);
  const url = vm.runInContext('atmoWindUrl({lat:37.77009,lon:-122.51363},1790222400.4)', context);
  assert.equal(url, 'https://sf.atmo.ai/wind@37.77009,-122.51363,10.77,20,0,1790222400,1.50');
  const link = app.match(/<a class="metric-link"[^>]+>/)?.[0] ?? '';
  assert.match(link, /target="_blank"/);
  assert.match(link, /rel="noreferrer"/);
  assert.match(link, /atmoWindUrl\s*\(\s*s\s*,\s*t\s*\)/);
});
